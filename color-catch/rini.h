...
float rini_get_config_valuef(rini_config config, const char* key)
{
    float value = 0;

	value = atof(rini_get_config_value_text(config, key));
...
int rini_set_config_valuef(rini_config* config, const char* key, float value, const char* desc)
{
    int result = -1;
    char value_text[RINI_MAX_TEXT_SIZE] = { 0 };
